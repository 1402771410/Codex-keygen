...
- 支持在临时邮箱规则中配置“POP3+Alias 无限邮箱”（主邮箱+随机后缀）
...
- 在“邮箱服务”页面新增规则时选择 `普通邮箱 POP3+Alias`
- 在规则内填写：主邮箱、POP3 服务器、端口、用户名、密码（或授权码）
- 注册页仅选择规则，不再要求重复手填 POP3 信息
- 系统会按 `主邮箱 + 随机后缀` 生成别名邮箱并通过 POP3 轮询提取验证码
...
cd Codex-keygen
python scripts/keygen.py install --mode auto
```
...
- Windows：`.\keygen.bat <subcommand>`
- Linux / macOS：`./keygen <subcommand>`（首次执行前：`chmod +x keygen`）

示例：

- `./keygen install` 等价于 `python scripts/keygen.py install --mode auto`
- `./keygen upgrade` 为兼容别名（等价于 `./keygen install`）
- `./keygen uninstall` 等价于 `python scripts/keygen.py uninstall --mode auto`
