...
let availableServices = {
    ...
    selection: { mode: 'single', single_service_id: null },
};
...
function updateEmailServiceOptions() {
    ...
    if (select.options.length === 0) {
        ...
        disabled.value = '';
        disabled.textContent = '暂无可用邮箱规则，请先到“邮箱服务”配置并完成测试';
...
function handleServiceChange(e) {
    ...
    if (type === 'tempmail') {
        ...
        addLog('warning', `[警告] 当前页面不支持邮箱类型: ${type}`);
    }
}
...
async function handleStartRegistration(e) {
    ...
    if (emailServiceType !== 'tempmail') {
        toast.error('当前页面仅支持通过“邮箱服务”规则进行注册');
        addLog('error', `[错误] 不支持的邮箱服务类型: ${emailServiceType}`);
        return;
    }
