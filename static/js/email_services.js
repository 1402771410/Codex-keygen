...
async function handleSaveGlobalTempmailSettings(event) {
    ...
    try {
        ...
        await loadGlobalTempmailSettings();
    } catch (error) {
...
async function handleTestGlobalTempmail() {
    ...
    try {
        const result = await api.post('/email-services/test-tempmail', { base_url: baseUrl });
        if (result.success) {
            ...
            <tr>
                <td colspan="7">
                    <div class="empty-state">
                        ...
                        <div class="empty-state-description">点击“添加规则”配置邮箱生成规则，注册会自动轮询启用规则。</div>
                    </div>
                </td>
            </tr>
        `;
